...
`speak.procterai.cc` is currently routed through the existing `ollama-tunnel` Cloudflare Tunnel to this Mac's local backend on `http://localhost:8788`. To make it fully cloud-hosted, update Cloudflare DNS for `speak.procterai.cc` from the current Cloudflare Tunnel CNAME to `choosing-to-speak-brain.onrender.com`, then verify the custom domain in Render.
